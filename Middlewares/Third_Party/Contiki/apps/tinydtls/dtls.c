...
{
#if defined(DTLS_ECC) || defined(DTLS_X509) //MGR
...
dtls_prepare_record(dtls_peer_t *peer, dtls_security_parameters_t *security,
  ...
  if (*rlen < DTLS_RH_LENGTH) {
    dtls_alert("The sendbuf (%lu bytes) is too small\n", *rlen);
    return dtls_alert_fatal_create(DTLS_ALERT_INTERNAL_ERROR);
  }
...
#if defined(DTLS_X509) || defined(DTLS_ECC) //MGR
static uint8 *
...
dtls_send_server_hello_msgs(dtls_context_t *ctx, dtls_peer_t *peer)
{
  ...
  if (is_tls_ecdhe_ecdsa_with_aes_128_ccm_8(peer->handshake_params->cipher)) {
    ...
    if (is_tls_ecdhe_ecdsa_with_aes_128_ccm_8(peer->handshake_params->cipher) &&
    		(is_ecdsa_client_auth_supported(ctx) || (is_x509_client_auth_supported(ctx)))) { //MGR
...
#endif /* defined(DTLS_ECC) || defined(DTLS_X509)  */ //MGR
...
dtls_send_client_hello(dtls_context_t *ctx, dtls_peer_t *peer,
  ...
  x509 = is_x509_supported(ctx, 1); //MGR (?)

  cipher_size = 2 + ((ecdsa || x509) ? 2 : 0) + (psk ? 2 : 0); //MGR
  extension_size = (ecdsa || x509) ? (2 + 6 + 6 + 8 + 6) : 0; //MGR
...
check_peer_certificate(dtls_context_t *ctx, //MGR
			 dtls_peer_t *peer,
			 uint8 *data, size_t data_length)
{
  int err;
			 ...
			 uint8 *data, size_t data_length)
{
  ...
  ret = CALL(ctx, verify_x509_cert, &peer->session, data, cert_length,
          config->keyx.ecdsa.other_pub_x, sizeof(config->keyx.ecdsa.other_pub_x),
          config->keyx.ecdsa.other_pub_y, sizeof(config->keyx.ecdsa.other_pub_y));
  if (ret < 0) {
    dtls_warn("The certificate was not accepted\n");
    return ret;
  }
...
#endif /* defined(DTLS_X509) || defined(DTLS_ECC) */ //MGR
...
handle_handshake_msg(dtls_context_t *ctx, dtls_peer_t *peer, session_t *session,
  ...
  case DTLS_HT_CERTIFICATE:
    ...
    //err = check_server_certificate(ctx, peer, data, data_length); //MGR
# ifdef DTLS_X509 //MGR
    if (CALL(ctx, is_x509_active) == 0)
...
# endif /* DTLS_X509 */
    ...
    break;
#endif /* defined(DTLS_ECC) || defined(DTLS_X509) */
